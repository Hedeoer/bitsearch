...
4. **Connect to backend services:** Import and call the appropriate service/repo functions. For provider-routed operations, use `runWithKeyPool` from `src/server/providers/fetch-router.ts`. For direct Grok calls, use `requireGrokConfig` to get credentials. For database access, use repo functions from `src/server/repos/`.
