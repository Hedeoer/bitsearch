...
Step-by-step instructions for setting up Grok, Tavily, and Firecrawl providers, configuring routing strategy, and managing API keys.
...
For each provider (`grok`, `tavily`, `firecrawl`), set:
- **enabled**: Toggle the provider on/off.
- **baseUrl**: API endpoint base URL (e.g., `https://api.x.ai/v1` for Grok, `https://api.tavily.com` for Tavily, `https://api.firecrawl.dev/v1` for Firecrawl).
- **timeoutMs**: Request timeout in milliseconds. Grok enforces a minimum of 120s for search; Firecrawl enforces a minimum of 60s for scrape.

## 2. Add API Keys

**Grok (single-key model):**
...
## 4. Configure Grok Model

Set the default Grok model via `system_settings` (key: `default_grok_model`). Default: `"grok-4-fast"`. Can be overridden per-request by passing a model parameter. Available models can be listed via `listGrokModels()` in `src/server/providers/grok-client.ts`.

## 5. Verify Configuration

- **Check provider status:** Call `getProviderConfig()` for each provider and confirm `enabled: true`, valid `baseUrl`, and `hasApiKey: true` (Grok) or `keyCount > 0` (Tavily/Firecrawl).
...
| Grok returns "未完整配置" error | Missing baseUrl or API key for Grok | Set both via `saveProviderConfig()` |
