...
- **Purpose:** Aggregates multiple search providers (Grok, Tavily, Firecrawl) behind a single MCP endpoint with automatic failover, API key pool management, and operational observability.
...
- **Multi-provider search routing:** Grok for AI-powered search; Tavily and Firecrawl for extraction, mapping, and supplemental search. Three fetch modes: `strict_tavily`, `strict_firecrawl`, `auto_ordered`.
